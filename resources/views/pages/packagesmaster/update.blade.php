...
{{-- page title --}}
@section('title',"Package $packagesMaster->name")
{{-- vendor style --}}
            ...
            <div class="card-content">
                <div class="card-title">
                    <div class="row center">
                        <div class="col s12">
                            <h5 class="center-align" style="font-weight: 900;">Package</h5>
                            <p>{{$packagesMaster->name}}</p>
                        </div>
                    </div>
                </div>

                <form  class="formValidate" action="{{ route('packages-master.update',$packagesMaster) }}" onsubmit="return verifyInclusion()" style="margin-bottom: 20px;" id="formSubmit" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row"></div>
                    <div id="details">
                        <div class="row">
                            <div class="input-field col s12">
                                ...
                                <input name="name" type="text" value="{{old('name', $packagesMaster->name)}}" required>
                                @error('name')
                        ...
                        <div class="row">
                            <div class="input-field col s12">
                                ...
                                <input name="code" type="text" value="{{old('code', $packagesMaster->code)}}" required>
                        ...
                        <div class="row">
                            <div class="col s5">
                                ...
                                <div class="input-field">
                                    ...
                                    <input  onkeypress="return isNumberKey(event)" value="{{old('stay_length',$packagesMaster->stay_length)}}" name="stay_length" id="stay_length" type="text" >
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="input-field col s12">
                                ...
                                <textarea required class="materialize-textarea"  name="description" type="text" >{{old('description',$packagesMaster->description)}}</textarea>
                                    @error('description')
                                        <small class="red-text">
                                            <p>{{ $message }}</p>
                                        </small>
                    ...
                    <div id="roomtypes" style="font-size: .7em;">
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                @foreach ($packagesMaster->packages_master_rooms as $package_room)
                                     @php
                                    ...
                                    <tr>
                                        ...
                                        <td style="padding: 5px !important; border: 1px solid #D5DBDB  !important; text-align: center; vertical-align: top;">
                                            <input name="room_type[{{$package_room->room_type->id}}][base_price]" onkeypress="return isNumberKey(event)" style="width:50px;" type="text" class="browser-default" value="{{$package_room->base_price}}">
                                        </td>
                                        <td style="padding: 5px !important; border: 1px solid #D5DBDB  !important; text-align: center; vertical-align: top;">
                                            <input name="room_type[{{$package_room->room_type->id}}][extra_person]" onkeypress="return isNumberKey(event)" style="width:50px;" type="text" class="browser-default" value="{{$package_room->extra_person}}">
                                            <br>
                                                <div id="upcharge{{$package_room->room_type->id}}" style="font-size: .8em">
                                                    @foreach ($package_room->packages_master_upcharges as $item)
                                        ...
                                        <td style="padding: 5px !important; border: 1px solid #D5DBDB  !important; text-align: center; vertical-align: top;">
                                            <input name="room_type[{{$package_room->room_type->id}}][extra_bed]" onkeypress="return isNumberKey(event)" style="width:50px;" type="text" class="browser-default" value="{{$package_room->extra_bed}}">
                                        </td>
                                        ...
                                        <td style="padding: 5px !important; border: 1px solid #D5DBDB  !important; vertical-align: top;">
                                            <select name="room_type[{{$package_room->room_type->id}}][adults_minimum]" class="browser-default">
                                                @for ($i = 0; $i <= $package_room->room_type->higher_occupancy; $i++)
                                                    <option {{$i == $package_room->adults_minimum? 'selected':''}} value="{{$i}}">{{$i}} {{Str::plural('Person', $i)}}</option>
                                                @endfor
                                            </select>
                                        </td>
                                        <td style="padding: 5px !important; border: 1px solid #D5DBDB  !important; vertical-align: top;">
                                            <select name="room_type[{{$package_room->room_type->id}}][children_minimum]" class="browser-default">
                                                @for ($i = 0; $i <= $package_room->room_type->higher_occupancy; $i++)
                                                    <option {{$i == $package_room->children_minimum? 'selected':''}}  value="{{$i}}">{{$i}} {{Str::plural('Child', $i)}}</option>
                                                @endfor
                                            </select>
                                        </td>
                                    </tr>
                        ...
                        <table id="inclusions" class="table">
                            ...
                            <tbody>
                                @php
                                    ...
                                    $contador =0;
                                @endphp
                                @foreach ($packagesMaster->inclusions as $inclusion)
                                     @php
                                        $total +=$inclusion->price_after_discount();
                                        $contador++;
                                    @endphp
                                    <tr id="inclusion{{$inclusion->id}}" >
                                        <td style="padding: 5px !important; border: 1px solid #D5DBDB  !important;">{{$contador}}</td>
                                        <td style="padding: 5px !important; border: 1px solid #D5DBDB  !important;">{{$inclusion->name}}</td>
                                        <td style="padding: 5px !important; border: 1px solid #D5DBDB  !important; text-align: right;">{{number_format($inclusion->price_after_discount(),2,".",",")}}<input class="inclusion_id" name="inclusion_id[]" type="hidden" value="{{$inclusion->id}}" /></td>
                                        <td class="invoice-action-edit mr-6"  style="padding: 5px !important; border: 1px solid #D5DBDB  !important;text-align: center; ">
                                ...
                                @endforeach
                                @if ($packagesMaster->inclusions)
                                    <tr><td style="padding: 5px !important; border: 1px solid #D5DBDB  !important;"></td>
                                        <td style="padding: 5px !important; border: 1px solid #D5DBDB  !important; text-align: center;"><button onclick="showInclusion()" type="button" class="btn btn-light-green" >Add more inclusions</button></td>
                                    <td style="padding: 5px !important; border: 1px solid #D5DBDB  !important; text-align: right;"><span id="total" data-total="{{$total}}">{{number_format($total,2,".",",")}}</span></td>
                                        <td style="padding: 5px !important; border: 1px solid #D5DBDB  !important;"></td>
                                    </tr>
                                @else
                                ...
                                @foreach ($hotel->inclusions as $inclusion)
                                    ...
                                    <tr>
                                        ...
                                        <td style="padding: 5px !important; border: 1px solid #D5DBDB  !important; text-align: center;">
                                            <input {{$packagesMaster->inclusions->contains('id', $inclusion->id)? 'checked':''}} id="check{{$inclusion->id}}" data-id="{{$inclusion->id}}" data-name="{{$inclusion->name}}" data-price="{{$inclusion->price_after_discount()}}"
