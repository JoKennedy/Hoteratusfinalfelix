...
{{-- page title --}}
@section('title','Update Room Tax/Fee')
{{-- vendor style --}}
    ...
    <div class="col s12">
      ...
      <div id="validations" class="card card-tabs">
        ...
        @if(Session::has('message_success'))
                        ...
                        @endif
            <form  class="formValidate" action="{{ route('room-taxes.update', $roomTax) }}" style="margin-bottom: 20px;" id="formSubmit" method="POST">
                @csrf
                @method('PUT')
                ...
                <div id="details">
                    <div class="row">
                        <div class="input-field col s8">
                            ...
                            <input name="name" type="text" value="{{old('name', $roomTax['name'])}}" required>
                            @error('name')
                         ...
                         <div class="input-field col s4">
                            ...
                            <input name="code" type="text" value="{{old('code', $roomTax['code'])}}" required>
                            @error('code')
                                <small class="red-text">
                                    <p>{{ $message }}</p>
                                </small>
                    ...
                    <div class="row">
                        ...
                        <div class="col s4">
                             <br><br><br>
                            <label>
                            <input  type="checkbox" id="included" name="included" {{$roomTax->included ? 'checked':''}} />
                    ...
                    <div class="row">
                        <div class="col s8">
                            ...
                            <div id="accouncode" class="input-field">
                                <select id="account_code_id" name="account_code_id" value="{{old('account_code_id', $roomTax->account_code_id)}}" required >
                        ...
                        <div class="col s4">
                            <br><br>
                            <div class="input-field" >
                                    <a id="add" style="display: none;" class="waves-effect waves-light btn-small add"><i class="material-icons left">add</i>Add New Account Code</a>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="input-field col s12">
                            ...
                            <textarea class="materialize-textarea" name="description">{{old('description', $roomTax->description)}}</textarea>
                            @error('description')
                                <small class="red-text">
                                    <p>{{ $message }}</p>
                                </small>
                    ...
                    <div class="row">
                       ...
                       <div class="row">
                           <div class="col s2">
                               <label>
                                    <input onclick="showOptions(this.value)"  class="radiobtn" value="2" name="type" type="radio" {{$roomTax->tax_applied->tax_type_id ==2? 'checked':''}} />
                                    <span><i  class="material-icons">attach_money</i></span>
                                </label>
                                <label>
                                    <input onclick="showOptions(this.value)"  class="radiobtn" value="1" name="type" type="radio" {{$roomTax->tax_applied->tax_type_id ==1? 'checked':''}}  />
                             ...
                             <table id="tablename" class="table">
                                ...
                                <tbody>
                                    ...
                                    <tr class="{{$key==0?'principal':''}}" id="{{$key}}">
                                         <td>
                                            <div class="input-field">
                                                <input class="limpiar" name="charge_less[]" onkeypress="return isNumberKey(event)" type="text" value="{{$details->charge_less ==0? '' : number_format($details->charge_less, 2, '.', ',') }}" />
                                            </div>
                                        </td>
                                         <td>
                                            <div id="accouncode">
                                        ...
                                        <td >
                                            <div class="input-field">
                                                <input class="limpiar" onkeypress="return isNumberKey(event)" name="tax_value[]" type="text"  value="{{ number_format($details->tax_value, 2, '.', ',') }}"  required>
                                            </div>
                                        </td>
                                    <td > <input class="deleterow" name="room_tax_detail_id[]" type="hidden" value="{{$details->id}}" /> <span><a data-token="{{ csrf_token() }}" onclick="removeTr(this)" href="javascript:void(0)"><i  class="material-icons">delete</i></a></span></td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
...
<script>
   ...
   $(document).ready(function () {
    ...
    function showOptions( type =1){
        var current = "{{old('tax_applied_id', $roomTax->tax_applied_id)}}";
        $('.taxoption').remove();
        ...
        $.ajax({
            ...
            success: function (response) {
                response.forEach(element => {
                     $("#tax_applied_id").append("<option "+(element.id==current?'selected':'')+' class="taxoption" value="'+element.id+'" >'+element.name+'</option>');
