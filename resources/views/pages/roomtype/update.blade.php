    ...
    <div class="col s12">
      ...
      <div id="validations" class="card card-tabs">
        ...
        <div class="card-content">
            ...
            <form enctype="multipart/form-data"  class="formValidate" action="{{ route('room-types.update', $roomType) }}" style="margin-bottom: 20px;" id="formSubmit" method="POST">
                @csrf
                @method('PUT')
                ...
                <div id="details">
                    <div class="row">
                        <div class="input-field col s8">
                            ...
                            <input name="name" type="text" value="{{old('name',$roomType->name)}}" required>
                            @error('name')
                        ...
                        <div class="input-field col s4">
                            ...
                            <input name="code" type="text" value="{{old('code',$roomType->code)}}" required>
                    ...
                    <div class="row">
                        <div class="input-field col s12">
                            ...
                            <textarea class="materialize-textarea"  name="description" type="text" >{{old('description',$roomType->description)}}</textarea>
                                @error('description')
                                    <small class="red-text">
                                        <p>{{ $message }}</p>
                                    </small>
                    ...
                    <div class="row">
                        <div class="col s6">
                            ...
                            <div id="accouncode" class="input-field">
                                <select id="base_occupancy" name="base_occupancy" value="{{old('base_occupancy',$roomType->base_occupancy)}}" required >
                                    @for ($i = 0; $i <= 50; $i++)
                                        <option value="{{$i}}" {{$i== old('base_occupancy',$roomType->base_occupancy)? 'selected' : '' }}>{{$i}}</option>
                                    @endfor
                        ...
                        <div class="col s6">
                            ...
                            <div id="accouncode" class="input-field">
                                <select id="higher_occupancy" name="higher_occupancy" value="{{old('higher_occupancy',$roomType->higher_occupancy)}}"  >
                                    @for ($i = 0; $i <= 50; $i++)
                                        <option value="{{$i}}" {{$i== old('higher_occupancy',$roomType->higher_occupancy)? 'selected' : '' }}>{{$i}}</option>
                                    @endfor
                    ...
                    <div class="row">
                        <div class="col s3">
                            ...
                            <input value="1" type="checkbox" {{old('exta_bed_allowed',$roomType->exta_bed_allowed) == 1? 'checked' : '' }} id="exta_bed_allowed" name="exta_bed_allowed" />
                                <span>Yes</span>
                            </label>
                        </div>
                         <div class="col s3">
                            ...
                            <div id="exta_bed_allowed_total" class="input-field">
                                ...
                                <select  name="exta_bed_allowed_total" value="{{old('exta_bed_allowed_total',$roomType->exta_bed_allowed_total)}}"  >
                                    @for ($i = 0; $i <= 50; $i++)
                                        <option value="{{$i}}" {{$i== old('exta_bed_allowed_total',$roomType->exta_bed_allowed_total)? 'selected' : '' }}>{{$i}}</option>
                                    @endfor
                                </select>
                            ...
                            <div class="input-field" >
                                <select name="amenity_id[]" class="select2-size-sm browser-default" multiple="multiple">
                                    @foreach ($amenities as $amenity)
                                        <option value="{{$amenity->id}}" {{old('amenity_id',$roomAmenities) && in_array($amenity->id,old('amenity_id',$roomAmenities) )? 'selected':''}} >{{$amenity->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <h6 class="text-center">Rack Rate (List price)*</h6>
                         <div class="input-field col s4">
                            ...
                            <input min="1" name="base_price" onkeypress="return isNumberKey(event)" type="text" value="{{old('base_price',$roomType->base_price)}}" required>
                            @error('base_price')
                                <small class="red-text">
                                    <p>{{ $message }}</p>
                                </small>
                        ...
                        <div class="input-field col s4">
                            ...
                            <input name="higher_price" onkeypress="return isNumberKey(event)" type="text" value="{{old('higher_price',$roomType->higher_price)}}" required>
                            @error('higher_price')
                                <small class="red-text">
                                    <p>{{ $message }}</p>
                                </small>
                        ...
                        <div class="input-field col s4">
                            ...
                            <input name="extra_bed_price" onkeypress="return isNumberKey(event)" type="text" value="{{old('extra_bed_price',$roomType->extra_bed_price)}}" required>
                      ...
                      <div class="row">
                        ...
                        @foreach ($taxes as $tax)
                            <div class="row">
                                <div class="col s12">
                                    <label>
                                    <input value="{{$tax->id}}" type="checkbox" {{old('room_tax_id', $roomTax) && in_array($tax->id,old('room_tax_id',$roomTax) )? 'checked':''}} name="room_tax_id[]"  />
                                    <span >
                                        <div>
