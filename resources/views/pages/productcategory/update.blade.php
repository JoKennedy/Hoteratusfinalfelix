        ...
        <div class="card-content">
            <div class="card-title">
                <div class="row center">
                    <div class="col s12">
                        <h5 class="center-align" style="font-weight: 900;">Category</h5>
                        <p>{{$category->name}}</p>
                    </div>
                </div>
            </div>
            ...
            @endif
            <form  class="formValidate" action="{{ route('productcategories.update', $category) }}" style="margin-bottom: 20px;" id="formSubmit" method="POST">
                @csrf
                @method('PUT')
                <div class="row"></div>
                <div id="details">
                     ...
                     <div class="row">
                        <div class="col s4">
                            ...
                            <div class="input-field">
                                <select onchange="filterAccountCode()" name="property_department_id" id="property_department_id" value="{{old('property_department_id', $category->property_department_id)}}">
                                    <option value="">Select a Department</option>
                                    @foreach ($departments as $department)
                                        <option value="{{$department->id}}" {{$department->id==old('property_department_id',$category->property_department_id)? 'selected':''}}>{{$department->name}}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col s4">
                            ...
                            <div class="input-field">
                                <select name="account_code_id" id="account_code_id" value="{{old('account_code_id', $category->account_code_id)}}">
                    ...
                    <div class="row">
                        <div class="input-field col s12">
                            ...
                            <textarea required class="materialize-textarea"  name="description" type="text" >{{old('description',$category->description)}}</textarea>
                                @error('description')
                                    <small class="red-text">
                                        <p>{{ $message }}</p>
                                    </small>
                ...
                <div class="row s12 right">
                    ...
                    <a href="{{ route('productcategories.index') }}" class="mb-6 btn waves-effect waves-light gradient-45deg-purple-deep-orange">Category List</a>
                </div>
            </form>
        </div>
      </div>
...
function filterAccountCode(){
    $(".loader").show();
    var aid = "{{$category->account_code_id}}";
