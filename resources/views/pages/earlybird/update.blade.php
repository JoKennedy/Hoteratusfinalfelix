...
{{-- page title --}}
@section('title','Update Early Bird')
{{-- vendor style --}}
        ...
        <div class="card-content">
            <div class="card-title">
                <div class="row center">
                    <div class="col s12">
                        <h5 class="center-align" style="font-weight: 900;">Early Bird</h5>
                        <p>{{$discountEarlyBird->name}}</p>
                    </div>
                </div>
            </div>

            <form  class="formValidate" action="{{ route('early-bird-discount.update', $discountEarlyBird) }}" style="margin-bottom: 20px;" id="formSubmit" method="POST">
                @csrf
                @method('PUT')
                <div class="row"></div>
                <div id="details">
                    <div class="row">
                        <div class="input-field col s12">
                            <label for="name">Early Bird Discount Name*</label>
                            <input name="name" type="text" value="{{old('name', $discountEarlyBird->name)}}" required>
                            @error('name')
                    ...
                    <table id="discountlist">
                        ...
                        <tbody>
                            @foreach ($discountEarlyBird->discount_early_bird_details as $key => $item )
                                <tr>
                                    <td style="text-align:center; padding: 5px;"><input  value="{{$item->start}}" style="text-align: center;"  onkeypress="return isNumberKey(event)"  name="start[]" type="text"  required> </td>
                                    <td style="text-align:center; padding: 5px;"><input  value="{{$item->end}}" style="text-align: center;" onkeypress="return isNumberKey(event)"  name="end[]" type="text"  required> </td>
                ...
                <div class="row s12 right">
                    ...
                    <a href="{{ route('early-bird-discount.index') }}" class="mb-6 btn waves-effect waves-light gradient-45deg-purple-deep-orange">Cancel</a>
                </div>
            </form>
        </div>
      </div>
