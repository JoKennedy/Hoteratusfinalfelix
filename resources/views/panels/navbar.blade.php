<div class="navbar @if(($configData['isNavbarFixed'])=== true){{'navbar-fixed'}} @endif">
  <nav
    ...
    <div class="nav-wrapper">
      ...
      <!-- notifications-dropdown    Aqui todas las notificaciones para este usuario-->
        ...
        <li>
          <a class="grey-text text-darken-1" href="{{ route('logout')  }}"
            onclick="event.preventDefault();
                                 document.getElementById('user-lock').submit();">
            <i class="material-icons">lock_outline</i>
            Lock
          </a>
           <form id="user-lock" action="{{ route('logout') }}" method="POST" style="display: none;">
            <input type="hidden"  name="userLock" class="hidden" value="true">
            @csrf
        </form>
        </li>
        <li>
    ...
    <nav class="display-none search-sm">
      <div class="nav-wrapper">
        <form id="navbarForm">
          ...
          <input class="search-box-sm mb-0" type="search" required="" placeholder='Explore {{env('APP_NAME')}}' id="search"
              data-search="template-list">
